...
const UnifiedEditModal: React.FC<Props> = ({ isOpen, onClose, type, id }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md flex flex-col p-6 gap-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between border-b pb-4">
                    <h2 className="text-xl font-bold">Edit {type === "device" ? "Device" : "Group"}</h2>
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded text-gray-500 hover:text-gray-700 hover:bg-gray-100 text-2xl leading-none pb-1" aria-label="Close modal">×</button>
                </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative shrink-0">
                                ...
                                {showColorPicker && createPortal(
                                    <div className="fixed inset-0 z-[9999] isolate">
                                        ...
                                        <div
                                            className="fixed z-[10000] bg-white p-2 rounded-lg shadow-xl border flex flex-col gap-2"
                                            style={{
                            ...
                            <input
                                ...
                                placeholder="Name"
                                className="flex-1 border rounded px-3 py-2 disabled:opacity-50 bg-white"
                                disabled={isLoading}
                            />
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {EMOJI_OPTIONS.map(opt => (
                                <button
                                    ...
                                    className={`w-9 h-9 flex items-center justify-center rounded hover:bg-white text-lg transition-all ${emoji === opt ? 'bg-white shadow ring-1 ring-blue-500' : ''}`}
                                    disabled={isLoading}
                                >
