...
function DeviceOverlayComponent({
  ...
  const { group, contributors, mostRecentSourceName } = useMemo(() => {
    const g = groupDevices.find((d) => d.id === chosen.device);
    const memberIds = g?.memberDeviceIds;

    if (!memberIds) {
      return { group: null, contributors: [], mostRecentSourceName: null };
    }

    const contribs = memberIds.map((id) => deviceNames[id] ?? `Device ${id}`);
    const latestId = memberIds.reduce((latest, id) => {
      const time = deviceLastSeen[id] ?? 0;
      const latestTime = latest ? (deviceLastSeen[latest] ?? 0) : -1;
      return time > latestTime ? id : latest;
    }, null as number | null);

    const latestName = latestId != null ? (deviceNames[latestId] ?? `Device ${latestId}`) : null;

    return { group: g, contributors: contribs, mostRecentSourceName: latestName };
  }, [chosen.device, groupDevices, deviceNames, deviceLastSeen]);
      ...
      <div className="flex items-start">
        <div className="flex-1">
          <div className="text-sm font-medium">
            {group ? group.name : (deviceNames[chosen.device] ?? `Device ${chosen.device}`)}
          </div>
