...
export class Engine {
  ...
  processMeasurements(ms: Measurement[]): EngineSnapshot[] {
    ...
    for (const m of measurements) {
      ...
      const compSnap = this.mixture.snapshot().map((c) => ({ mean: [c.mean[0], c.mean[1]] as [number, number], cov: [c.cov[0], c.cov[1], c.cov[2]], weight: c.weight, source: c.source, action: (c as any).action, spawnedDuringMovement: (c as any).spawnedDuringMovement, createdAt: (c as any).createdAt }));
